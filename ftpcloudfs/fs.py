...
class ObjectStorageFD(object):
    ...
    def upload_manifest(self):
        ...
        if self.storage_policy is not None:
            self.headers.update({'x-storage-policy': quote(self.storage_policy)})
        logging.debug("creating manifest %r/%r, headers=%r, contents=%s" % (self.container, self.name, self.headers, contents))
        try:
            self.conn.put_object(self.container, self.name, headers=self.headers, contents=contents, query_string="multipart-manifest=put")
        except ClientException as ex:
